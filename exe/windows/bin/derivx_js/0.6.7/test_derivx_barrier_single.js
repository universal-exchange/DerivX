...
function CalcPayoff(module, s, h, k, x, v, r, q, t, p, is_call, is_kop_delay, barrier_type) {
    let inputs = {'s':s, 'h':h, 'k':k, 'x':x, 'v':v, 'r':r, 'q':q, 't':t, 'p':p, 'is_call':is_call, 'is_kop_delay':is_kop_delay, 'barrier_type':barrier_type}
    let result = JSON.parse(module.DirectCall(0, func_calc_payoff, 0, JSON.stringify(inputs)))
    if(result['return_code'] !== 0) {
        console.log(result['return_code'], result['return_info'])
    }
    return result['result_data']
...
function Test_DerivX_Barrier_Single() {
    ...
    let module = new cyberx.Create('derivx_barrier_single') // 全局唯一
    //let module_01 = new cyberx.Create('derivx_barrier_single') // 重复创建会报异常
    //let module_01 = new cyberx.GetCreate('derivx_barrier_single') // 可以获取已创建的实例
    ...
    */
    ...
    //let calc_price = [75.0, 80.0, 85.0, 90.0, 95.0, 100.0, 105.0, 110.0, 115.0] // 计算价格序列
