...
def CalcIV(module, model, method, p, s, k, r, q, t, is_call, tree_step = 10000):
    ...
    result = json.loads(module.DirectCall(0, func_calc_iv, 0, json.dumps(inputs)))
...
def CalcGreeksSurface(module, model, greek, array_s, k, r, q, v, array_t, is_long = True, is_call = True, is_futures = False, is_foreign = False, tree_step = 10000):
    ...
    result = json.loads(module.DirectCall(0, func_calc_greeks_surface, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]
...
def Test_DerivX_Vanilla_European():
    ...
    module = cyberx.Create("derivx_vanilla_european") # 全局唯一
    #module_01 = cyberx.Create("derivx_vanilla_european") # 重复创建会报异常
    #module_01 = cyberx.GetCreate("derivx_vanilla_european") # 可以获取已创建的实例
