...
g_up_in    = 1 # 向上敲入
g_down_in  = 2 # 向下敲入
g_up_out   = 3 # 向上敲出 # 看涨情况下，与同为一年期敲出收益为零的欧式看涨单鲨区别蛮大，后者与万得的同期欧式认购出局单向障碍验证一致
g_down_out = 4 # 向下敲出 # 看跌情况下，与同为一年期敲出收益为零的欧式看跌单鲨区别蛮大，后者与万得的同期欧式认沽出局单向障碍验证一致
...
def CalcPrice(module, s, h, k, x, v, r, q, t, p, is_call, is_kop_delay, barrier_type):
    ...
    result = json.loads(module.DirectCall(0, func_calc_price, 0, json.dumps(inputs)))
...
def CalcPayoff(module, s, h, k, x, v, r, q, t, p, is_call, is_kop_delay, barrier_type):
    inputs = {"s":s, "h":h, "k":k, "x":x, "v":v, "r":r, "q":q, "t":t, "p":p, "is_call":is_call, "is_kop_delay":is_kop_delay, "barrier_type":barrier_type}
    result = json.loads(module.DirectCall(0, func_calc_payoff, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]

def CalcGreeks(module, calc_greek, s, h, k, x, v, r, q, t, p, is_call, is_futures, is_foreign, is_kop_delay, barrier_type, calc_price, runs_step, year_days, run_from, run_days):
    inputs = {"calc_greek":calc_greek, "s":s, "h":h, "k":k, "x":x, "v":v, "r":r, "q":q, "t":t, "p":p, "is_call":is_call, "is_futures":is_futures, "is_foreign":is_foreign, 
    ...
    result = json.loads(module.DirectCall(0, func_calc_greeks, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]
...
def Test_DerivX_Barrier_Single():
    ...
    module = cyberx.Create("derivx_barrier_single") # 全局唯一
    #module_01 = cyberx.Create("derivx_barrier_single") # 重复创建会报异常
    #module_01 = cyberx.GetCreate("derivx_barrier_single") # 可以获取已创建的实例
