...
def Gap_CalcPrice(module, s, k_1, k_2, r, q, v, t, is_call):
    ...
    result = json.loads(module.DirectCall(0, func_gap_calc_price, 0, json.dumps(inputs)))
...
def Gap_CalcPayoff(module, s, k_1, k_2, is_call):
    ...
    result = json.loads(module.DirectCall(0, func_gap_calc_payoff, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]
...
def CashOrNothing_CalcPrice(module, s, k, r, q, v, t, cash, is_call):
    ...
    result = json.loads(module.DirectCall(0, func_cash_or_nothing_calc_price, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]
...
def CashOrNothing_CalcPayoff(module, s, k, cash, is_call):
    ...
    result = json.loads(module.DirectCall(0, func_cash_or_nothing_calc_payoff, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]
...
def AssetOrNothing_CalcPrice(module, s, k, r, q, v, t, is_call):
    ...
    result = json.loads(module.DirectCall(0, func_asset_or_nothing_calc_price, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]
...
def AssetOrNothing_CalcPayoff(module, s, k, is_call):
    ...
    result = json.loads(module.DirectCall(0, func_asset_or_nothing_calc_payoff, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]
...
def SuperShare_CalcPrice(module, s, k_l, k_h, r, q, v, t):
    ...
    result = json.loads(module.DirectCall(0, func_super_share_calc_price, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]
...
def SuperShare_CalcPayoff(module, s, k_l, k_h):
    ...
    result = json.loads(module.DirectCall(0, func_super_share_calc_payoff, 0, json.dumps(inputs)))
    if result["return_code"] != 0:
        print(result["return_code"], result["return_info"])
    return result["result_data"]
...
def Test_Derivx_Digital_Simple():
    ...
    module = cyberx.Create("derivx_digital_simple") # 全局唯一
    #module_01 = cyberx.Create("derivx_digital_simple") # 重复创建会报异常
    #module_01 = cyberx.GetCreate("derivx_digital_simple") # 可以获取已创建的实例
