...
function CalcIV(module, model, method, p, s, k, r, q, t, is_call, tree_step = 10000) {
    ...
    let result = JSON.parse(module.DirectCall(0, func_calc_iv, 0, JSON.stringify(inputs)))
...
function CalcPrice(module, model, s, k, r, q, v, t, is_call, tree_step = 10000) {
    ...
    let result = JSON.parse(module.DirectCall(0, func_calc_price, 0, JSON.stringify(inputs)))
    if(result['return_code'] !== 0) {
        console.log(result['return_code'], result['return_info'])
    }
    return result['result_data']
...
function CalcGreeks(module, model, greek, s, k, r, q, v, t, is_long = true, is_call = true, is_futures = false, is_foreign = false, tree_step = 10000) {
    ...
    let result = JSON.parse(module.DirectCall(0, func_calc_greeks, 0, JSON.stringify(inputs)))
    if(result['return_code'] !== 0) {
        console.log(result['return_code'], result['return_info'])
    }
    return result['result_data']
...
function CalcGreeksSurface(module, model, greek, array_s, k, r, q, v, array_t, is_long = true, is_call = true, is_futures = false, is_foreign = false, tree_step = 10000) {
    ...
    let result = JSON.parse(module.DirectCall(0, func_calc_greeks_surface, 0, JSON.stringify(inputs)))
    if(result['return_code'] !== 0) {
        console.log(result['return_code'], result['return_info'])
    }
    return result['result_data']
...
function Test_DerivX_Vanilla_European() {
    ...
    let module = new cyberx.Create('derivx_vanilla_european') // 全局唯一
    //let module_01 = new cyberx.Create('derivx_vanilla_european') // 重复创建会报异常
    //let module_01 = new cyberx.GetCreate('derivx_vanilla_european') // 可以获取已创建的实例
    ...
    //result = CalcPrice(module, 'bs', 100.0, 100.0, 0.03, 0.08 - 0.03, 0.2, 1.0, true)
    ...
    //result = CalcPrice(module, 'bs', 100.0, 100.0, 0.03, 0.08 + 0.03, 0.2, 1.0, false)
