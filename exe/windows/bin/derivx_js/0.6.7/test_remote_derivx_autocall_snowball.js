...
async function Test_DerivX_Autocall_Snowball() {
    ...
    tasker_test.timeout_wait = 5 // 秒 // 时间设短一点可测试任务取消操作
    ...
    //for(let [name, flag] of Object.entries(greek_flags)) {
        ...
        //result = kernel.AssignTask(tasker_test) // 同步 # 超时会自动取消任务
        //if(result['return_code'] !== 0) {
        //    console.log(result['return_code'], result['return_info'])
        //}
        //else {
        ...
        //    if(ret_wait != true) {
        //        console.log('等待任务结果超时！' + tasker_id)
        //        cancel = kernel.CancelTask(tasker_id) // 只用同步方式
        //        console.log(cancel['return_code'], cancel['return_info'])
        //    } else {
        //        cancel = kernel.CancelTask(tasker_id) // 只用同步方式
        //        console.log(cancel['return_code'], cancel['return_info'])
        //    }
