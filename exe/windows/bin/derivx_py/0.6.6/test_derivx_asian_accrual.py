...
class Config(object):
    def __init__(self):
        ...
        self.notional = 0.0 # 名义本金，目前未使用，为 start_price * multiplier
        self.trade_long = False # 交易方向
        self.accrual_type = 0 # 累计类型
        self.start_price = 0.0 # 标的价格，具体价格点位
        self.strike_price = 0.0 # 执行价格
        self.option_fee = 0.0 # 期权费费率，CalcPrice 时此入参不参与计算
        self.option_fee_interest = 0.0 # 期权费利率
        self.option_fee_abs_need = False # 期权费费率是否为绝对，False 为年化，True 为绝对
        self.back_end_load = False # 期权费支付方式，False 为前端，True 为后端
        self.multiplier = 0.0 # 头寸乘数，目前未使用，即相当于名义份额
        self.realized_price = [] # 已实现的标的价格序列
...
def Test_DerivX_Asian_Accrual():
    ...
    config.realized_price = np.array([]).tolist() # 已实现的标的价格序列
    #config.realized_price = np.array([101.3, 102.5, 101.0, 100.0, 98.9, 98.5, 97.3, 98.9, 99.9]).tolist() # 已实现的标的价格序列
