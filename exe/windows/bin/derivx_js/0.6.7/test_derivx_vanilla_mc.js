/*
* Copyright (c) 2021-2026 the DerivX authors
* All rights reserved.
*
...
// 示例说明：
// 1、演示香草（蒙卡）期权结构参数设置；
// 2、演示香草（蒙卡）期权结构价格、收益曲面、希腊值曲面等的计算；
// 3、演示 tasker 任务信息创建；
// 4、演示 同步模式 和 异步模式 的 AssignTask 任务执行调用；
// 5、演示异步回调函数的编写和使用；

// 注意：模型 CalcPrice 计算所得的价格（期权费）可选年化或绝对，比率，非百分比，非绝对金额。
// 注意：期权费率年化模式，对于相同的一年期的美式期权，看涨的结果和使用 BA 或 BT 方法计算所得的数值基本一致，看跌的结果尤其是深度实值的因为提前行权年化天数缩减导致数值差异会非常大。
...
let g_option_call = 1 // 看涨
let g_option_put = 2 // 看跌
...
async function Test_DerivX_Vanilla_MC() {
    ...
    config.option_type = g_option_european // 期权类型
    config.vanilla_type = g_option_call // 香草类型，1 看涨，2 看跌
    config.strike_payoff_abs = true // 行权时的收益是年化还是绝对，false 为年化，true 为绝对
    config.start_price = 100.0 // 初始价格，具体价格点位
    config.strike_price = 110.0 // 执行价格，具体价格点位
    
    config.option_fee = 0.035 // 期权费费率，默认年化，CalcPrice 时此入参不参与计算
    config.option_fee_interest = 0.03 // 期权费利率
    config.option_fee_abs_need = false // 期权费费率是否为绝对，False 为年化，True 为绝对
    ...
    tasker_test.plugin_id = 'derivx_vanilla_mc'
