...
let g_accrual_uc   = 1 // 看涨
let g_accrual_dp   = 2 // 看跌
...
class Config {
    constructor() {
        ...
        this.notional = 0.0 // 名义本金，目前未使用，为 start_price * multiplier
        this.trade_long = false // 交易方向
        this.accrual_type = 0 // 累计类型
        this.start_price = 0.0 // 初始价格，具体价格点位
        this.strike_price = 0.0 // 执行价格
        this.option_fee = 0.0 // 期权费费率，CalcPrice 时此入参不参与计算
        this.option_fee_interest = 0.0 // 期权费利率
        this.option_fee_abs_need = false // 期权费费率是否为绝对，false 为年化，true 为绝对
        this.back_end_load = false // 期权费支付方式，false 为前端，true 为后端
        this.multiplier = 0.0 // 头寸乘数，目前未使用，即相当于名义份额
        this.realized_price = [] // 已实现的标的价格序列
...
async function Test_DerivX_Asian_Accrual() {
    ...
    config.basis_rate = 0.017 // 股息或贴水 // InitPath
    config.risk_free_rate = 0.017 // 无风险利率 // InitPath
    config.price_limit_ratio = 0.1 // 涨跌停限制幅度 // InitPath
    config.price_limit_style = 0 // 涨跌停限制方式，0 不限制，1 超限部分移至下日，2 超限部分直接削掉 // InitPath
    
    config.notional = 100.0 * 200.0 * 1000.0 // 名义本金，目前未使用，为 start_price * multiplier
    config.trade_long = false // 交易方向
    config.accrual_type = g_accrual_uc // 累计类型
    config.start_price = 100.0 // 初始价格，具体价格点位
    config.strike_price = 100.0 // 执行价格
    config.option_fee = 0.0266 // 期权费费率，CalcPrice 时此入参不参与计算
    config.option_fee_interest = 0.017 // 期权费利率
    config.option_fee_abs_need = true // 期权费费率是否为绝对，false 为年化，true 为绝对
    config.back_end_load = false // 期权费支付方式，false 为前端，true 为后端
    config.multiplier = 200.0 * 1000.0 // 头寸乘数，目前未使用，即相当于名义份额
    config.realized_price = nj.array([]).tolist() // 已实现的标的价格序列
    //config.realized_price = nj.array([101.3, 102.5, 101.0, 100.0, 98.9, 98.5, 97.3, 98.9, 99.9]).tolist() // 已实现的标的价格序列
    ...
    config.discount_rebate = true // 是否对返息进行贴现，影响后端返息，false 为不贴现，true 为做贴现
    config.compound_rebate = true // 是否对返息进行复利，影响前端返息，false 为不复利，true 为做复利
    ...
    tasker_test.plugin_id = 'derivx_asian_accrual'
