...
class Config(object):
    def __init__(self):
        ...
        self.knock_i_max_loss = 0.0 # 是否敲入后限制最大亏损比率，非百分比，1.0 为不限亏，小于 1.0 则变成不追保凤凰
        self.knock_i_participate = 0.0 # 敲入参与率，非百分比，不影响 knock_i_max_loss 敲入后限制最大亏损比率
        self.knock_i_margin_call = True # 是否敲入后可追加保证金，置为 False 则变成不追保凤凰 # 目前此参无效，已被 knock_i_max_loss 取代
        self.knock_i_only_at_end = False # 是否仅在期末观察敲入
        self.knock_i_above_get = False # 是否在期末获得高于敲入价的绝对收益 # 针对锁盈缓冲结构，期末不观察敲出只观察敲入，需设置高敲出比率不发生敲出 # 目前定为仍正常计算付息
...
def Test_DerivX_Autocall_Phoenix():
    ...
    config.knock_i_max_loss = 1.0 # 是否敲入后限制最大亏损比率，非百分比，1.0 为不限亏，小于 1.0 则变成不追保凤凰
    config.knock_i_participate = 1.0 # 敲入参与率，非百分比，不影响 knock_i_max_loss 敲入后限制最大亏损比率
    config.knock_i_margin_call = True # 是否敲入后可追加保证金，置为 False 则变成不追保凤凰 # 目前此参无效，已被 knock_i_max_loss 取代
    config.knock_i_only_at_end = False # 是否仅在期末观察敲入
    config.knock_i_above_get = False # 是否在期末获得高于敲入价的绝对收益 # 针对锁盈缓冲结构，期末不观察敲出只观察敲入，需设置高敲出比率不发生敲出 # 目前定为仍正常计算付息
    ...
    config.knock_o_days = np.array([20, 40, 61, 81, 101, 122, 142, 162, 183, 203, 223, 244]).tolist() # 敲出观察日(交易日)
    ...
    #config.knock_o_days_n = np.array([30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330, 360]).tolist() # 敲出日期序列(自然日) (可选)
