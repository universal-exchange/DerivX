...
class Config {
    constructor() {
        ...
        this.knock_i_max_loss = 0.0 // 是否敲入后限制最大亏损比率，非百分比，1.0 为不限亏，小于 1.0 则变成不追保凤凰
        this.knock_i_participate = 0.0 // 敲入参与率，非百分比，不影响 knock_i_max_loss 敲入后限制最大亏损比率
        this.knock_i_margin_call = true // 是否敲入后可追加保证金，置为 false 则变成不追保凤凰 // 目前此参无效，已被 knock_i_max_loss 取代
        this.knock_i_only_at_end = false // 是否仅在期末观察敲入
        this.knock_i_above_get = false // 是否在期末获得高于敲入价的绝对收益 // 针对锁盈缓冲结构，期末不观察敲出只观察敲入，需设置高敲出比率不发生敲出 // 目前定为仍正常计算付息
        this.knock_o_observe_pay = false // 是否在敲出观察日支付票息，false 为累计至敲出或到期支付，true 为每次当日支付
...
async function Test_DerivX_Autocall_Phoenix() {
